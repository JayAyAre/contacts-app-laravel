@extends('layouts.app')

@section('content')
  <div class="container">
    <h3 class="text-center mb-4">Contacts shared with me</h3>
    @forelse($contactsSharedWithUser as $contact)
      <div class="d-flex justify-content-between bg-dark mb-3 rounded px-4 py-2">
        <div>
          <a href="{{route("contacts.show",$contact->id)}}">
            <img class="profile_picture" src="{{Storage::url($contact->profile_picture)}}">
          </a>
        </div>
        <div class="d-flex align-items-center">
          <p class="me-2 mb-0">Name: {{ $contact->name }}</p>
          <p class="me-2 mb-0 d-none d-md-block">Phone:<a
                href="tel:{{ $contact->phone_number }}"> {{ $contact->phone_number }}</a></p>
          <p class="me-2 mb-0 d-none d-md-block">Email:<a
                href="mailto:{{ $contact->email }}"> {{ $contact->email }}</a></p>
          <p class="me-2 mb-0">Age: {{ $contact->age }}</p>
          <p class="me-2 mb-0"><span class="text-info">Shared by: {{ $contact->user->email }}</span></p>
        </div>
      </div>
    @empty
      <div class="col-md-4 mx-auto">
        <div class="card car-body text-center mb-3">
          <p>No shared contacts with u</p>
        </div>
      </div>
    @endforelse
    <div class="container">
      <h3 class="text-center mb-4">Contacts shared by me</h3>
      @forelse($contactsSharedByUser as $contact)
        @foreach($contact->sharedWithUsers as $user)
          <div class="d-flex justify-content-between bg-dark mb-3 rounded px-4 py-2">
            <div>
              <a href="{{route("contacts.show",$contact->id)}}">
                <img class="profile_picture" src="{{Storage::url($contact->profile_picture)}}">
              </a>
            </div>
            <div class="d-flex align-items-center">
              <p class="me-2 mb-0">Name: {{ $contact->name }}</p>
              <p class="me-2 mb-0 d-none d-md-block">Phone:<a
                    href="tel:{{ $contact->phone_number }}"> {{ $contact->phone_number }}</a></p>
              <p class="me-2 mb-0 d-none d-md-block">Email:<a
                    href="mailto:{{ $contact->email }}"> {{ $contact->email }}</a></p>
              <p class="me-2 mb-0">Age: {{ $contact->age }}</p>
              <p class="me-2 mb-0"><span class="text-info">Shared with: {{ $user->email }}</span></p>
              <form action="{{route("contact-shares.destroy",$user->pivot->id)}}" method="POST">
                @csrf
                @method("DELETE")
                <button type="submit" class="btn btn-danger mb-0 p-1 px-2">
                  unshare
                </button>
              </form>
            </div>
          </div>
        @endforeach
      @empty
        <div class="col-md-4 mx-auto">
          <div class="card car-body text-center">
            <p>You have no shared contacts</p>
            <p><a href="{{route("contact-shares.create")}}">Share one</a></p>
          </div>
        </div>
      @endforelse
    </div>
@endsection
